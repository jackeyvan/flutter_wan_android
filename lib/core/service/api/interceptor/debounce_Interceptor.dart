...
class DebounceInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    final isShowLoadingDialog = headers['is_show_loading_dialog'] != null &&
        headers['is_show_loading_dialog'] == 'true';

    if (headers['network_debounce'] != null &&
        headers['network_debounce'] == 'true') {
      //需要处理请求防抖去重
      parameters = _generateParameters(method, options);
      _handleNetworkDebounce(url, method, parameters, cancelToken, options,
          handler, isShowLoadingDialog);
    } else {
      if (isShowLoadingDialog) {
        /// TODO SmartDialog.showLoading();
      }
      // 处理去重
  ...
  //根据请求方式生成不同的参数格式
  Map<String, dynamic>? _generateParameters(
      String method, RequestOptions options) {
    if (method == 'GET') {
  ...
  void _handleNetworkDebounce(
    ...
    if (params == null) {
      ...
      Log.d(
          "请求前先查询 _urlParamsMap 集合目前的数据:${_urlParamsMap.toString()} _cancelTokenMap：${_cancelTokenMap.toString()}");
      final preSerializedParams = _urlParamsMap[urlkey];
      final curSerializedParams = _serializeAllParams(params);
      Log.d(
          "已缓存的请求参数Value cacheedValue:${preSerializedParams.toString()} 当前正在发起的请求的参数Value:${curSerializedParams.toString()}");
  ...
  String _generateCacelKey(
      String method, String url, Map<String, dynamic>? map) {
  ...
  void handleEndWithRequestOption(RequestOptions requestOptions) {
    ...
    final isShowLoadingDialog =
        requestHeaders['is_show_loading_dialog'] != null &&
            requestHeaders['is_show_loading_dialog'] == 'true';

    if (requestHeaders['network_debounce'] != null &&
        requestHeaders['network_debounce'] == 'true') {
      //请求完成之后移除CancelToken，和 Params Map
      final url = requestOptions.uri.path;
      final method = requestOptions.method;
      Map<String, dynamic>? params =
          _generateParameters(method, requestOptions);
    ...
    if (isShowLoadingDialog) {
      /// TODO SmartDialog.dismiss(status: SmartStatus.loading);
    }
  }
